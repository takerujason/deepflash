...
#               loss = self.criterion(output, img[:,0:1,:,:,:])
                ...
                if (j%5 == 0) and (i == len(training_dataloader)-1):
                    ...
                    # image = nib.load('./prediction_0.nii')
                    # header_info = image.header
                    # print(header_info)
                    # header_info['pixdim'][1:4]  = [3,17,17]
                    # # c = np.array(image.get_fdata())
                    # # Use the new `header_info` before writing
                    # x = nib.Nifti1Image(pred, image.affine, header_info)
                    # nib.save(x, 'something.nii.gz')
                    sitk.WriteImage(im, './prediction_%s.nii'%j, True)
                    # im.to_filename(os.path.join('build','test4d.nii.gz'))
