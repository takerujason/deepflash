...
# dependencies
sys.path.append('../')
from configs.networks import Sample,RescaleDouble,ResizeDouble,Negate
from configs.losses import networkfuncs
...
def register(gpu_id, moving, fixed, model_file, out_img, out_warp):
    ...
    with tf.device(gpu):
        ...
        custom_objects = {'SpatialTransformer': nrn_layers.SpatialTransformer,
                 ...
                 'Sample': Sample,
                 'Rescale': RescaleDouble,
                 'Resize': ResizeDouble,
                 'Negate': Negate,
                 'recon_loss': networkfuncs(0.02, 10).loss1, 
                 'kl_loss': networkfuncs(0.02, 10).loss2        
                 }
